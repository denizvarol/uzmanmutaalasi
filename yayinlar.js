/**
 * UZMAN İNCELEME - FİZİKSEL YAYINLAR VERİ DOSYASI
 * C:\Users\User\Desktop\Projeler\Danışmanlık\yayinlar.js
 * 
 * Kullanıcı tarafından bizzat gönderilen toplam 12 yayın:
 * - 6 Adli Mühendislik (Fabrikalar Artık Sadece Yanmıyor Makale + Poster, Mahkemede Gördüğünüz Şeye İnanabilir misiniz? Makale + Poster, Amazon-UEI Makale + Poster)
 * - 2 Hasar & Sigorta (Endüstriyel Hasar ve Sigorta Mühendisliği Makale + Poster)
 * - 4 Adli Hesap Uzmanlığı (Mahkemede Rakam, 4 Günlük İzin, Endeks Hatası, Bir Tarih)
 */

window.YAYINLAR_DATA = [
  {
    "id": "art-fabrikalar-siber-hasar-makale",
    "title": "Fabrikalar Artık Sadece Yanmıyor, Verileri de Çalınıyor (Makale)",
    "category": "makale",
    "uzmanlik": "muhendislik",
    "author": "Melike Özlem BİLGİLİ",
    "date": "12 Ağustos 2026",
    "readTime": "10 dk okuma",
    "image": "assets/vaka_analizi_thumb.png",
    "fileName": "fabrikalar_siber_hasar_makalesi.pdf",
    "fileUrl": "assets/docs/fabrikalar_siber_hasar_makalesi.pdf",
    "hasFile": true,
    "summary": "Sanayide yeni hasar türü: Fiziksel kayıptan siber-fiziksel zarara, IT/OT yakınsaması, KVKK ihlal ilanları, 7545 sayılı Siber Güvenlik Kanunu ve 12 noktalı adli mühendislik matrisi.",
    "content": "Sanayi tesislerinde klasik hasar dosyası yangın, patlama ve makine kırılması üzerinden kurulurdu. Dijitalleşme bu tabloyu değiştirdi. Günümüzde bir siber olay kişisel veriye erişimle sınırlı kalmayıp IT/OT yakınsaması nedeniyle operasyon teknolojilerini etkileyerek siber-fiziksel kayba ve iş durmasına dönüşebilir. Makale, yeni hasar kavramını 6 katmanlı adli mühendislik modeliyle ele almaktadır."
  },
  {
    "id": "art-fabrikalar-siber-hasar-poster",
    "title": "Fabrikalar Artık Sadece Yanmıyor, Verileri de Çalınıyor (İnteraktif Poster)",
    "category": "rehber",
    "uzmanlik": "muhendislik",
    "author": "Melike Özlem BİLGİLİ",
    "date": "12 Ağustos 2026",
    "readTime": "3 dk okuma",
    "image": "assets/fabrikalar_siber_hasar_poster_thumb.jpg",
    "fileName": "fabrikalar_siber_hasar_poster.jpg",
    "fileUrl": "assets/docs/fabrikalar_siber_hasar_poster.jpg",
    "hasFile": true,
    "summary": "Sanayide Yeni Hasar Türü: 5,50 Milyon $ Ortalama İhlal Maliyeti, Yeni Hasarın 6 Katmanı ve 12 Noktalı Sanayi Siber Hasar İnceleme Matrisi (İnfografik Poster).",
    "content": "Yeni fabrika hasarı bazen duman çıkarmadan başlar; ancak üretimi, veriyi ve bilançoyu aynı anda vurabilir. 12 noktalı adli mühendislik matrisi ve yönetim için 10 kritik soruyu içeren infografik poster."
  },
  {
    "id": "art-dijital-delil-deepfake-makale",
    "title": "Mahkemede Gördüğünüz Şeye Artık İnanabilir misiniz? (Makale)",
    "category": "makale",
    "uzmanlik": "muhendislik",
    "author": "Melike Özlem BİLGİLİ",
    "date": "12 Ağustos 2026",
    "readTime": "9 dk okuma",
    "image": "assets/uzman_makalesi_thumb.png",
    "fileName": "dijital_delil_deepfake_makalesi.pdf",
    "fileUrl": "assets/docs/dijital_delil_deepfake_makalesi.pdf",
    "hasFile": true,
    "summary": "Yapay zekâ çağında dijital delil, deepfake, AB AI Act Madde 50 şeffaflık kuralları, C2PA Content Credentials ve 5 katmanlı adli doğrulama çerçevesi.",
    "content": "Bir dijital kayıt artık yalnızca ne gösterdiği ile değil, nasıl üretildiği, nereden geldiği ve hangi bağımsız izlerle doğrulandığı ile değerlendirilmek zorunda. Üretken yapay zekâ çağında adli bilişim ve adli mühendislik, verinin kökenini, bütünlüğünü ve gerçekliğini 12 noktalı denetim zinciri ile sınar."
  },
  {
    "id": "art-dijital-delil-deepfake-poster",
    "title": "Mahkemede Gördüğünüz Şeye Artık İnanabilir misiniz? (İnteraktif Poster)",
    "category": "rehber",
    "uzmanlik": "muhendislik",
    "author": "Melike Özlem BİLGİLİ",
    "date": "12 Ağustos 2026",
    "readTime": "3 dk okuma",
    "image": "assets/dijital_delil_poster_thumb.jpg",
    "fileName": "dijital_delil_deepfake_poster.jpg",
    "fileUrl": "assets/docs/dijital_delil_deepfake_poster.jpg",
    "hasFile": true,
    "summary": "Yapay Zekâ ve Deepfake Çağında Dijital Delil Güvenliği, 5 Ayrı Doğrulama Katmanı ve 12 Kontrol Noktası (İnfografik Poster).",
    "content": "En değerli delil görüntünün kendisi olmayabilir. Asıl değer; içeriğin hangi sistemden geldiğini, nasıl edinildiğini ve bağımsız kayıtlarla ne ölçüde doğrulandığını gösteren teknik izler bütünündedir. İnfografik adli mühendislik posteri."
  },
  {
    "id": "art-amazon-uei-makale",
    "title": "Amazon–UEI Davası: Akıllı Ev Teknolojisinin Görünmeyen Sınırları (Makale)",
    "category": "makale",
    "uzmanlik": "muhendislik",
    "author": "Melike Özlem BİLGİLİ",
    "date": "12 Ağustos 2026",
    "readTime": "10 dk okuma",
    "image": "assets/vaka_analizi_thumb.png",
    "fileName": "amazon_uei_akilli_ev_makalesi.pdf",
    "fileUrl": "assets/docs/amazon_uei_akilli_ev_makalesi.pdf",
    "hasFile": true,
    "summary": "Echo ve Fire TV üzerindeki patent iddiaları, akıllı bina adli mühendisliği, IoT mimarisi, istem-ürün eşlemesi ve dijital delil metodolojisi üzerinden ayrıntılı teknik inceleme.",
    "content": "Universal Electronics Inc. (UEI), 6 Ağustos 2026'da ABD Kaliforniya Federal Mahkemesinde Amazon.com, Inc. ve AWS aleyhine patent ihlali davası açtı. Bu makale, doğrulanmış dava olgularını patent adli mühendisliği, IoT mimarisi, claim chart mantığı ve dijital delil metodolojisi üzerinden 11 maddelik matris ile ayrıntılı şekilde inceler."
  },
  {
    "id": "art-amazon-uei-poster",
    "title": "Amazon–UEI Davası: Akıllı Ev Teknolojisinin Görünmeyen Sınırları (İnteraktif Poster)",
    "category": "rehber",
    "uzmanlik": "muhendislik",
    "author": "Melike Özlem BİLGİLİ",
    "date": "12 Ağustos 2026",
    "readTime": "3 dk okuma",
    "image": "assets/amazon_uei_poster_thumb.jpg",
    "fileName": "amazon_uei_akilli_ev_poster.jpg",
    "fileUrl": "assets/docs/amazon_uei_akilli_ev_poster.jpg",
    "hasFile": true,
    "summary": "12 Noktalı Adli Teknik İnceleme Matrisi, 6 Katmanlı Bilirkişi İnceleme Modeli ve Akıllı Ev Patent İhlali Özeti (İnfografik Poster).",
    "content": "Akıllı evde tek bir cihaz yoktur. Donanım, yazılım, protokol ve bulut aynı delil zincirindedir. 12 noktalı adli teknik matris ve 6 katmanlı bilirkişi analizini içeren infografik teknik poster."
  },
  {
    "id": "art-hasar-sigorta-makale",
    "title": "Endüstriyel Hasar ve Sigorta Mühendisliği (Makale)",
    "category": "makale",
    "uzmanlik": "hasar",
    "author": "Melike Özlem BİLGİLİ",
    "date": "12 Ağustos 2026",
    "readTime": "10 dk okuma",
    "image": "assets/mevzuat_notu_thumb.png",
    "fileName": "endustriyel_hasar_sigorta_makalesi.pdf",
    "fileUrl": "assets/docs/endustriyel_hasar_sigorta_makalesi.pdf",
    "hasFile": true,
    "summary": "Fabrika yangınından üretim kaybına: Teknik neden, fiziksel zarar, kök neden analizi, rayiç/ikame değer ve sigorta tazminatı nasıl birbirinden ayrılır?",
    "content": "Endüstriyel bir hasar olayı tek bir rakam değildir. Yangın, patlama, elektriksel arıza, makine kırılması ve proses kazası aynı anda fiziksel varlık kaybı, üretim kesintisi, ek işletme gideri, sovtaj ve rücu potansiyeli doğurabilir. Bu makale; teknik nedensellik, NFPA 921 prensipleri, iş durması (BI) ve 12 noktalı endüstriyel hasar mühendisliği matrisini inceler."
  },
  {
    "id": "art-hasar-sigorta-poster",
    "title": "Endüstriyel Hasar ve Sigorta Mühendisliği (İnteraktif Poster)",
    "category": "rehber",
    "uzmanlik": "hasar",
    "author": "Melike Özlem BİLGİLİ",
    "date": "12 Ağustos 2026",
    "readTime": "3 dk okuma",
    "image": "assets/endustriyel_hasar_poster_thumb.jpg",
    "fileName": "endustriyel_hasar_sigorta_poster.jpg",
    "fileUrl": "assets/docs/endustriyel_hasar_sigorta_poster.jpg",
    "hasFile": true,
    "summary": "Küresel Hasar Verileri (%21 Yangın, %36 İş Durması), Kök Neden - Hasar - Tazminat Zinciri ve 12 Noktalı İdeal Veri Seti (İnfografik Poster).",
    "content": "Hasarın bedeli yanan şeylerin toplamı değildir. Nedensellik + teknik değer + üretim etkisi + poliçe sınırı birlikte okunur. 12 noktalı ideal veri seti ve 6 katmanlı teknik değerlendirme içeren infografik poster."
  },
  /*
  TEMPORARILY HIDDEN FOR LAUNCH - ADLİ HESAP ARTICLES (CAN BE RESTORED ANYTIME)
  {
    "id": "art-hesap-rakam-delil",
    "title": "Mahkemede Rakam Tek Başına Delil Değildir (Uzman Makalesi)",
    "category": "makale",
    "uzmanlik": "hesap",
    "author": "Talip BİLGİLİ",
    "date": "12 Ağustos 2026",
    "readTime": "8 dk okuma",
    "image": "assets/hesap_thumb_1.png",
    "fileName": "adli_hesap_mahkemede_rakam.pdf",
    "fileUrl": "assets/docs/adli_hesap_mahkemede_rakam.pdf",
    "hasFile": true,
    "summary": "Adli hesap uzmanlığı ve nitelikli hesaplamalar: Uyuşmazlığın rakama dönüştüğü yerde bilirkişilik, 12 yüksek değerli uyuşmazlık kümesi ve denetlenebilir hesap zinciri.",
    "content": "Adli hesap raporlaması, tarafların ileri sürdüğü rakamları yeniden toplamak değildir. Nitelikli hesaplama; hukuken belirlenmiş veya mahkemece kabul edilmesi gereken parametreleri, doğrulanabilir mali ve ticari verilerle birleştirerek denetlenebilir bir sonuca dönüştürür. Adalet Bakanlığı Bilirkişilik Daire Başkanlığının güncel sınıflandırmasında Nitelikli Hesaplamalar 60 numaralı temel uzmanlık alanıdır."
  },
  {
    "id": "art-hesap-dort-gun-izin",
    "title": "Dört Günlük Hesap Hatası Kararı Bozdurdu (Uzman Makalesi)",
    "category": "makale",
    "uzmanlik": "hesap",
    "author": "Talip BİLGİLİ",
    "date": "12 Ağustos 2026",
    "readTime": "7 dk okuma",
    "image": "assets/hesap_thumb_2.png",
    "fileName": "adli_hesap_dort_gunluk_hesap_hatasi.pdf",
    "fileUrl": "assets/docs/adli_hesap_dort_gunluk_hesap_hatasi.pdf",
    "hasFile": true,
    "summary": "Yıllık izin hesabında takvim, ispat ve bilirkişi metodolojisi (Yargıtay 9. Hukuk Dairesi E.2025/9525, K.2026/757 sayılı kanun yararına bozma kararı incelemesi).",
    "content": "Yargıtay 9. Hukuk Dairesinin 3 Şubat 2026 tarihli kararı, yıllık izin hesabında küçük görünen bir takvim hatasının yargısal sonucu nasıl değiştirebildiğini gösteren güncel bir örnektir. 28 günlük izin belgelerindeki 4 hafta tatili gününün 4857 sayılı İş Kanunu m.56/5 uyarınca yıllık izinden sayılamayacağı ve bakiye 4 gün iznin bulunduğu hükme bağlanmıştır."
  },
  {
    "id": "art-hesap-endeks-fiyat-farki",
    "title": "Bir Endeks Hatası Milyonları Değiştirebilir (Uzman Makalesi)",
    "category": "makale",
    "uzmanlik": "hesap",
    "author": "Talip BİLGİLİ",
    "date": "12 Ağustos 2026",
    "readTime": "9 dk okuma",
    "image": "assets/hesap_thumb_3.png",
    "fileName": "adli_hesap_endeks_hatasi.pdf",
    "fileUrl": "assets/docs/adli_hesap_endeks_hatasi.pdf",
    "hasFile": true,
    "summary": "Kamu projelerinde hakediş, fiyat farkı ve adli hesaplamanın görünmeyen gücü (KİK 2025/2026 mevzuat değişiklikleri ve 12 noktalı adli hesap matrisi).",
    "content": "Kamu yapım ve hizmet sözleşmelerinde hakediş, yalnızca gerçekleşen imalat miktarı ile sözleşme fiyatının çarpımı değildir. Fiyat farkı öngörülen işlerde baz endeks, güncel endeks, ağırlık katsayıları, uygulama ayı, revize iş programı ve süre uzatımı gibi parametreler parasal sonucu doğrudan değiştirir."
  },
  {
    "id": "art-hesap-tarih-faiz-oran",
    "title": "Bir Tarih Milyonluk Fark Yaratabilir (Uzman Makalesi)",
    "category": "makale",
    "uzmanlik": "hesap",
    "author": "Talip BİLGİLİ",
    "date": "12 Ağustos 2026",
    "readTime": "8 dk okuma",
    "image": "assets/hesap_thumb_4.png",
    "fileName": "adli_hesap_bir_tarih_milyonluk_fark.pdf",
    "fileUrl": "assets/docs/adli_hesap_bir_tarih_milyonluk_fark.pdf",
    "hasFile": true,
    "summary": "Faiz hesabında yanlış oran, yanlış başlangıç tarihi ve 1 Eylül 2026 eşiği (Kanuni faiz %24, ticari temerrüt %39,75, TTK m.1530 %43 incelemesi).",
    "content": "Faiz hesabı, ana paraya bir yüzde yazıp gün sayısıyla çarpmaktan ibaret değildir. Alacağın hukuki kaynağı, temerrüt tarihi, faiz türü, sözleşmesel oran, ticari iş niteliği, dönemsel oran değişiklikleri ve kısmi ödemeler sonucu doğrudan etkiler. 7589 sayılı Kanunla 1 Eylül 2026'dan itibaren TCMB reeskont oranına bağlı değişken faiz sistemine geçilmektedir."
  }
  */
];
